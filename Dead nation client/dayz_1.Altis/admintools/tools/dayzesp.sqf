...
if (isNil "I_fuck_Animals") then {I_fuck_Animals = true;};  		//Show animals on start
...
if (isNil "alternate_tp") then {alternate_tp = true;};		//Use alternative teleport (could be used to tp on buildings)(not works for vehicles)(credits to monky)
...
who_is_UDA = "I'm here";  					//Text to identify You on map/gps. Leave blank ("") if You want to see only black icon
...
_bigASSmap = 20000;				//Size of map (longest x or y) in meters
...
drawic = 
{
	if (!isnil "mapm") then 
	{
		...
		if _zombie_R_gay then {{if (!isnull _x) then {_ctrl drawIcon ["\A3\ui_f\data\map\vehicleicons\iconMan_ca.paa", zmc, getPosASL _x, _iscale*30, _iscale*30, getDir _x, "", 1];};} foreach zombies;};
		if I_fuck_Animals then {{if (!isnull _x) then {_ctrl drawIcon ["\A3\ui_f\data\map\vehicleicons\iconAnimal_ca.paa", amc, getPosASL _x, _iscale*15, _iscale*15, 0, "", 1];};} foreach animalsl;};
		if show_tentz then {{if (!isnull _x) then {_ctrl drawIcon ["\A3\ui_f\data\map\vehicleicons\iconobject_ca.paa", tmc, getPosASL _x, 30, 30, 0, "", 1];};} foreach tents;};
		if show_dead_lolz then {{if ((!isnull _x) and !(_x isKindOf "zZombie_base") and (_x isKindOf "Man")) then {_ctrl drawIcon ["\A3\ui_f\data\map\mapcontrol\Cross_CA.paa", amc, getPosASL _x, _iscale*30, _iscale*30, 0, "", 1];};} foreach AllDead;};
	
		{if  (!isnull _x) then {_ctrl drawIcon ["\A3\Air_F_Beta\Heli_Attack_01\Data\UI\Map_Heli_Attack_01_CA.paa", wmc, getPosASL _x, 30, 30, 0, "", 1];};} foreach helicrashes;
		{
			if (!isnull _x) then 
			{
	...
	
		{
			...
			{	
				if (_x == player) then {_color = [0, 0, 0, 1];} else {_color = pmc;};
		       		_ctrl drawIcon ["\A3\ui_f\data\map\vehicleicons\iconMan_ca.paa", _color, getPosASL _x, _iscale*30, _iscale*30, getDir _x, "", 1];
		        };
...
{	
	...
	{				
		...
		{
			_pos = _this select 0;
			if alternate_tp then {_dummyveh = "C_Rubberboat" createVehicleLocal [_pos select 0, _pos select 1];player moveInDriver _dummyveh;deleteVehicle _dummyveh;}
