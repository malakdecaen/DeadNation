...
debugLog format ["GEAR_FILTER_ handlegear %1", _this];
...
private ["_event","_data","_display","_control","_primaryWeaponControl","_IDCsArray","_lbIDC","_lbColumn"];
/*
...
_lbSelectedItem = {
  ...
  _selectedItem = lnbData [_lbIDC,[_index,1]];
  _selectedItem;
};
...
_arrowsEnable = {
  ...
  _index = lbCurSel _lbcontrol;
  _selectedItemCountLeft = lnbText [_lbIDC,[_index,0]];
  _selectedItemCountRight = lnbText [_lbIDC,[_index,2]];
};
...
for [{_i=0},{_i<count(_IDCsArray)}, {_i = _i + 1} ] do {
  ...
  for [{_j=0},{_j<count(_slotArray)}, {_j = _j + 1} ] do {
    ...
    _primaryWeaponControl ctrlCommit 0.3;  
  };
};
};
_higlightIDCs = {
...
_highlightGearSlots ={
  ...
  debuglog _selectedItemSlotFlags;
...
_setFilterIcon = {
debugLog format ["GEAR_FILTER_ setFilterIcon %1 %2", _this, (_icons select IGUI_GEAR_activeFilter)];  
...
_setFilterGlobals = {
  //_activeFilter = {true,false,false,false,false,false,false};
  _activeFilter  = _this;
  IGUI_GEAR_filterPrimaryWeapon = (_activeFilter select 0);
  IGUI_GEAR_filterSecondaryWeapon = (_activeFilter select 1);
  IGUI_GEAR_filterAmmo = (_activeFilter select 2);
